...
func (d *Driver) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if value, ok := volumeParams[ProvisioningMode]; ok {
		...
		// Use backward compatibility validator for provisioning mode validation
		validator := NewBackwardCompatibilityValidator(d)
		if err := validator.ValidateProvisioningModeParameter(provisioningMode); err != nil {
			return nil, err
		}
	} else {
	...
	// Handle efs-ns provisioning mode
	if provisioningMode == NamespaceMode {
		return d.createEFSNSVolume(ctx, req, volumeParams)
	}
...
// createEFSNSVolume handles volume creation for efs-ns provisioning mode
func (d *Driver) createEFSNSVolume(ctx context.Context, req *csi.CreateVolumeRequest, volumeParams map[string]string) (*csi.CreateVolumeResponse, error) {
	klog.V(4).Infof("CreateEFSNSVolume: called with args %+v", util.SanitizeRequest(*req))

	_ = req.GetName()                             // volName not used in efs-ns mode
	_ = req.GetCapacityRange().GetRequiredBytes() // volSize not used in efs-ns mode

	// Extract namespace from volume parameters
	namespace, ok := volumeParams[Namespace]
	if !ok || namespace == "" {
		// Fallback to PvcNamespace for backward compatibility
		namespace, ok = volumeParams[PvcNamespace]
		if !ok || namespace == "" {
			return nil, status.Errorf(codes.InvalidArgument, "Missing or empty %v parameter for efs-ns mode", Namespace)
		}
	}

	// Extract cluster ID from volume parameters
	clusterID, ok := volumeParams[ClusterID]
	if !ok || clusterID == "" {
		return nil, status.Errorf(codes.InvalidArgument, "Missing or empty %v parameter for efs-ns mode", ClusterID)
	}
	...
	// Parse and validate StorageClass parameters for efs-ns mode
	fsOptions, err := parseEFSNSStorageClassParameters(volumeParams)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "Invalid storage class parameters: %v", err)
	}

	// Validate filesystem options
	if err := fsOptions.Validate(); err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "Invalid filesystem options: %v", err)
	}

	// Get cloud client
	_, roleArn, crossAccountDNSEnabled, err := getCloud(req.GetSecrets(), d)
	if err != nil {
		return nil, err
	}

	// Check if namespace filesystem manager is available
	if d.namespaceManager == nil {
		return nil, status.Error(codes.Internal, "Namespace filesystem manager not initialized")
	}

	// Create or get filesystem for namespace
	fsInfo, err := d.namespaceManager.CreateOrGetFileSystemForNamespace(ctx, namespace, fsOptions)
	if err != nil {
		if efsnsErr, ok := err.(*efsns.EFSNSError); ok {
			switch efsnsErr.Type {
			case efsns.ErrFileSystemCreationFailed:
				return nil, status.Errorf(codes.Internal, "Failed to create filesystem for namespace %s: %v", namespace, err)
			case efsns.ErrNamespaceValidationFailed:
				return nil, status.Errorf(codes.InvalidArgument, "Namespace validation failed: %v", err)
			default:
				return nil, status.Errorf(codes.Internal, "Filesystem operation failed: %v", err)
			}
		}
		return nil, status.Errorf(codes.Internal, "Failed to create or get filesystem for namespace %s: %v", namespace, err)
	}

	// Generate volume ID for efs-ns format: efs-ns::{namespace}::{filesystem-id}::{cluster-id}
	volumeID := &efsns.EFSNSVolumeID{
		Namespace:    namespace,
	...
	// Add PVC to tracker
	if d.pvcTracker != nil {
		err = d.pvcTracker.AddPVC(ctx, namespace, volumeParams[PvcName], volumeID.String())
		if err != nil {
			klog.Warningf("Failed to add PVC to tracker: %v", err)
			// Don't fail the request - tracker is for optimization
		}
	}

	// Add finalizer to ensure cleanup
	if d.finalizerManager != nil {
		// Note: In a real implementation, we'd get the PVC object from Kubernetes API
		// For now, we'll add the finalizer during volume mounting in the node service
		klog.V(4).Infof("PVC finalizer will be added during node publish volume")
	}

	// Prepare volume context
	volContext := map[string]string{}

	// Handle cross-account mounting for efs-ns
	if roleArn != "" {
		if crossAccountDNSEnabled {
			volContext[CrossAccount] = strconv.FormatBool(true)
		} else {
			// For efs-ns, we'll use the first available mount target
			if len(fsInfo.MountTargets) > 0 {
				volContext[MountTargetIp] = fsInfo.MountTargets[0].IPAddress
			} else {
				klog.Warningf("No mount targets found for filesystem %s", fsInfo.FileSystemID)
			}
		}
	}

	// Add encryption in transit setting to volume context
	if encryptInTransit, ok := volumeParams[EncryptInTransit]; ok {
		volContext[EncryptInTransit] = encryptInTransit
	} else {
		// Default to true for efs-ns mode
		volContext[EncryptInTransit] = strconv.FormatBool(efsns.DefaultEncryptInTransit)
	}

	klog.V(2).Infof("Created EFS-NS volume %s for namespace %s using filesystem %s", volumeID.String(), namespace, fsInfo.FileSystemID)
...
// deleteEFSNSVolume handles volume deletion for efs-ns provisioning mode
func (d *Driver) deleteEFSNSVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	klog.V(4).Infof("DeleteEFSNSVolume: called with args %+v", util.SanitizeRequest(*req))

	volId := req.GetVolumeId()

	// Parse efs-ns volume ID
	volumeID, err := efsns.ParseEFSNSVolumeID(volId)
	if err != nil {
		// Return success for invalid volume ID as per CSI spec
		klog.V(5).Infof("DeleteEFSNSVolume: Failed to parse efs-ns volume ID: %v, err: %v, returning success", volId, err)
		return &csi.DeleteVolumeResponse{}, nil
	}

	namespace := volumeID.Namespace
	fileSystemID := volumeID.FileSystemID

	klog.V(4).Infof("DeleteEFSNSVolume: Deleting volume %s for namespace %s, filesystem %s", volId, namespace, fileSystemID)

	// Check if namespace filesystem manager is available
	if d.namespaceManager == nil {
		return nil, status.Error(codes.Internal, "EFS-NS components not initialized")
	}

	// Remove PVC from tracker
	if d.pvcTracker != nil {
		// Extract PVC name from volume ID if possible, or use a placeholder
		// In production, this would come from the PVC object
		pvcName := fmt.Sprintf("pvc-%s", volumeID.FileSystemID) // Fallback name
		_, err = d.pvcTracker.RemovePVC(ctx, namespace, pvcName)
		if err != nil {
			return nil, status.Errorf(codes.Internal, "Failed to remove PVC from tracker: %v", err)
		}
	}

	// Delete filesystem for namespace
	err = d.namespaceManager.DeleteFileSystemForNamespace(ctx, namespace, volId)
	if err != nil {
		if efsnsErr, ok := err.(*efsns.EFSNSError); ok {
			switch efsnsErr.Type {
			case efsns.ErrFileSystemDeletionFailed:
				klog.Warningf("Failed to delete filesystem for namespace %s: %v", namespace, err)
				// Don't fail the deletion request - the filesystem might have been manually deleted
				return &csi.DeleteVolumeResponse{}, nil
			case efsns.ErrNamespaceValidationFailed:
				klog.V(5).Infof("Namespace validation failed during deletion: %v", err)
				return &csi.DeleteVolumeResponse{}, nil
			default:
				klog.Warningf("Filesystem deletion operation failed: %v", err)
				return &csi.DeleteVolumeResponse{}, nil
			}
		}
		klog.Warningf("Failed to delete filesystem for namespace %s: %v", namespace, err)
		// Don't fail the deletion - be lenient with cleanup errors
		return &csi.DeleteVolumeResponse{}, nil
	}

	klog.V(2).Infof("Successfully deleted EFS-NS volume %s for namespace %s", volId, namespace)
	return &csi.DeleteVolumeResponse{}, nil
}
...
func (d *Driver) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	// Validate volume ID format using backward compatibility validator
	validator := NewBackwardCompatibilityValidator(d)
	if err := validator.ValidateVolumeID(volId); err != nil {
		// Log validation error but continue with deletion attempt for backward compatibility
		klog.V(4).Infof("DeleteVolume: Volume ID validation failed: %v, continuing with deletion", err)
	}

	// Check if this is an efs-ns volume ID
	if strings.HasPrefix(volId, efsns.VolumeIDPrefix+efsns.VolumeIDSeparator) {
		return d.deleteEFSNSVolume(ctx, req)
	}
...
func (d *Driver) ValidateVolumeCapabilities(ctx context.Context, req *csi.ValidateVolumeCapabilitiesRequest) (*csi.ValidateVolumeCapabilitiesResponse, error) {
	...
	// Check if this is an efs-ns volume ID
	if strings.HasPrefix(volId, efsns.VolumeIDPrefix+efsns.VolumeIDSeparator) {
		// Parse efs-ns volume ID to validate format and extract namespace
		volumeInfo, err := efsns.ParseEFSNSVolumeID(volId)
		if err != nil {
			return nil, status.Errorf(codes.NotFound, "EFS-NS volume not found, invalid volume ID: %v", err)
		}

		// Validate that the filesystem exists for this namespace
		if d.namespaceManager != nil {
			_, err := d.namespaceManager.GetFileSystemInfo(ctx, volumeInfo.Namespace)
			if err != nil {
				if efsns.IsEFSNSError(err, efsns.ErrFileSystemNotFound) {
					return nil, status.Errorf(codes.NotFound, "EFS-NS volume not found, filesystem does not exist for namespace %s", volumeInfo.Namespace)
				}
				return nil, status.Errorf(codes.Internal, "Failed to validate EFS-NS volume: %v", err)
			}
		}

		// EFS-NS volumes support the same capabilities as regular EFS volumes
	...
	// Handle regular efs-ap volume IDs
...
// parseEFSNSStorageClassParameters parses StorageClass parameters for efs-ns mode
func parseEFSNSStorageClassParameters(volumeParams map[string]string) (*efsns.FileSystemOptions, error) {
	...
	if value, ok := volumeParams[PerformanceMode]; ok {
		if value != "generalPurpose" && value != "maxIO" {
			return nil, fmt.Errorf("invalid performance mode: %s, must be 'generalPurpose' or 'maxIO'", value)
		}
		options.PerformanceMode = value
	}

	// Parse throughput mode
	if value, ok := volumeParams[ThroughputMode]; ok {
		if value != "bursting" && value != "provisioned" {
			return nil, fmt.Errorf("invalid throughput mode: %s, must be 'bursting' or 'provisioned'", value)
		}
		options.ThroughputMode = value
	}
	...
	if value, ok := volumeParams[ProvisionedThroughputInMibps]; ok {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid provisioned throughput value: %s, must be a positive integer", value)
		}
		if throughput <= 0 {
			return nil, fmt.Errorf("provisioned throughput must be positive, got: %d", throughput)
		}
		options.ProvisionedThroughputInMibps = &throughput
	}

	// Validate throughput mode and provisioned throughput consistency
	if options.ThroughputMode == "provisioned" && options.ProvisionedThroughputInMibps == nil {
		return nil, fmt.Errorf("provisioned throughput mode requires provisionedThroughputInMibps parameter")
	}
	if options.ThroughputMode == "bursting" && options.ProvisionedThroughputInMibps != nil {
		return nil, fmt.Errorf("bursting throughput mode cannot have provisionedThroughputInMibps parameter")
	}
	...
	if value, ok := volumeParams[Encrypted]; ok {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid encrypted value: %s, must be 'true' or 'false'", value)
		}
	...
	if value, ok := volumeParams[KmsKeyId]; ok {
		if options.Encrypted != nil && *options.Encrypted {
			options.KmsKeyID = &value
		} else {
			return nil, fmt.Errorf("kmsKeyId parameter requires encrypted to be true")
		}
	}

	// Copy any additional tags from volume parameters
	for key, value := range volumeParams {
		if strings.HasPrefix(key, "tag:") {
			tagKey := strings.TrimPrefix(key, "tag:")
			options.Tags[tagKey] = value
		}
	}
...
// validateNamespaceName validates Kubernetes namespace name according to RFC 1123
func validateNamespaceName(namespace string) error {
	...
	if len(namespace) > 63 {
		return fmt.Errorf("namespace name too long: %d characters, maximum 63 allowed", len(namespace))
	}

	// Kubernetes namespace names must be valid DNS-1123 labels
	// They can contain lowercase letters, numbers, and hyphens
	// Must start and end with alphanumeric character
	if !strings.HasPrefix(namespace, strings.ToLower(namespace)) {
		return fmt.Errorf("namespace name must be lowercase")
	}
	...
	for i, r := range namespace {
		if i == 0 || i == len(namespace)-1 {
			...
			if !((r >= 'a' && r <= 'z') || (r >= '0' && r <= '9')) {
				return fmt.Errorf("namespace name must start and end with alphanumeric character")
			}
		} else {
			// Middle characters can be alphanumeric or hyphen
			if !((r >= 'a' && r <= 'z') || (r >= '0' && r <= '9') || r == '-') {
				return fmt.Errorf("namespace name can only contain lowercase letters, numbers, and hyphens")
			}
		}
	}
