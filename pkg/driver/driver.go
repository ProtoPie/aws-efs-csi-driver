...
type Driver struct {
	...
	// EFS-NS components
	namespaceManager efsns.NamespaceFileSystemManager
	pvcTracker       efsns.PVCTracker
	finalizerManager efsns.FinalizerManager
}
...
// SetEFSNSComponents sets up the EFS-NS components for namespace-based provisioning
func (d *Driver) SetEFSNSComponents(namespaceManager efsns.NamespaceFileSystemManager, pvcTracker efsns.PVCTracker, finalizerManager efsns.FinalizerManager) {
	d.namespaceManager = namespaceManager
	d.pvcTracker = pvcTracker
	d.finalizerManager = finalizerManager
	klog.V(4).Infof("EFS-NS components initialized successfully")
}
