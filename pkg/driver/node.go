...
func (d *Driver) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	var fsid, vpath, apid string
	var err error

	// Check if this is an efs-ns volume ID
	if isEFSNSVolumeId(req.GetVolumeId()) {
		// For efs-ns volumes, we mount the entire filesystem at root
		fsid, err = parseEFSNSVolumeForMount(req.GetVolumeId())
		if err != nil {
			return nil, err
		}
		// efs-ns volumes always mount at root path, no access points
		subpath = "/"
		vpath = ""
		apid = ""
		klog.V(4).Infof("NodePublishVolume: detected efs-ns volume ID %s, using filesystem %s", req.GetVolumeId(), fsid)
	} else {
		// Handle traditional efs-ap volumes
	...
	// For efs-ns volumes, encryptInTransit is enabled by default unless explicitly disabled
	if isEFSNSVolumeId(req.GetVolumeId()) {
		if encryptInTransit {
	...
	if m := volCap.GetMount(); m != nil {
		for _, f := range m.MountFlags {
			...
			if strings.HasPrefix(f, "accesspoint=") {
				// For efs-ns volumes, access points should not be used
				if isEFSNSVolumeId(req.GetVolumeId()) {
					return nil, status.Error(codes.InvalidArgument,
						"access point mount options are not supported with efs-ns volumes, which manage entire filesystems per namespace")
				}
...
func (d *Driver) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	// Log different handling for efs-ns vs efs-ap volumes
	if isEFSNSVolumeId(req.GetVolumeId()) {
		klog.V(4).Infof("NodeUnpublishVolume: handling efs-ns volume %s", req.GetVolumeId())
	} else {
		klog.V(4).Infof("NodeUnpublishVolume: handling efs-ap volume %s", req.GetVolumeId())
	}
...
// isEFSNSVolumeId checks if the given volume ID is in efs-ns format
func isEFSNSVolumeId(volumeId string) bool {
	return strings.HasPrefix(volumeId, efsns.VolumeIDPrefix+efsns.VolumeIDSeparator)
}

// parseEFSNSVolumeForMount parses an efs-ns volume ID and returns the filesystem ID for mounting
func parseEFSNSVolumeForMount(volumeId string) (fsid string, err error) {
	efsnsVolumeId, err := efsns.ParseEFSNSVolumeID(volumeId)
	if err != nil {
		return "", status.Error(codes.InvalidArgument, fmt.Sprintf("Invalid efs-ns volume ID: %v", err))
	}

	// For efs-ns volumes, we mount the entire filesystem at root path
	fsid = efsnsVolumeId.FileSystemID
	return fsid, nil
}
